...
よって、**コード編集のために最適化されたエディタ**（コードエディタ）を利用することが強く推奨される。
...
- ウェブ界隈ではほぼ**デファクトスタンダード（事実上の標準）**（2023 年現在）
  - <small>プロもアマチュアも皆使っている。使い方を身につけておけば今後も役に立つはず</small>
...
```sh
git clone https://github.com/amekusa/code_the_web.git ~/Desktop/code_the_web_2023
```
3. *デスクトップ*に *`code_the_web_2023`* フォルダができているか確認
4. *VS Code* で `code_the_web_2023` フォルダを開く
`ファイル` → `フォルダーを開く` → `デスクトップ` → `code_the_web_2023`

`code_the_web_2023` フォルダには今見ているスライドも含まれているので、いつでも好きな時に参照してください。場所: `01/README.html`
